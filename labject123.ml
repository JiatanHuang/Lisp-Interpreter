(*CSCI 2041 labject123 Jiatan Huang && Ziyue Zhuang*)
(*  
...
val printThing :thing -> unit(*function makePrinter*)
(*BadThing. Raised if a thing cannot be printed*)
exception BadThing
end ;;
...
module type Lispers =
sig
  val repl : unit -> unit (*function makeLisper*)
  (*Can'tParse. Raised if can't read a thing*)
  exception Error
end;;
...
let repl() = 
commandArguments (fun path -> try 
let nextThing = Parser.makeParser path (*Make a parser that reads Lisp expressions from p*)
  in let rec lisping () = (*Read a Lisp expression from p*)
  let thing = nextThing() 
in
    match thing with
      Symbol ("end") -> () | (*If the expression is the symbol end, then stop*)
      _ -> 
        Printer.printThing (Evaluator.evaluate thing); (*Evaluate the expression to obtain a value.*)(*Print that value.*)
            lisping ()
    in lisping () 
  with (*Exceptions*)
